...
- All three originally-deferred aggregations (`avg_pick_time`, `active_deliveries`, `inventory_velocity`) are now implemented. `avg_pick_time` is proven correct via unit tests and a `TestStream`-based test, but has not been confirmed live against real Pub/Sub — DirectRunner appears not to fire merging (Session) windows against an unbounded source within practical test durations, even though non-merging `FixedWindows` fire fine under the same watermark (see NOTES.md 2026-07-24 entries). Worth retesting if this pipeline is ever deployed to actual Dataflow. Next open items: deploying to Dataflow instead of DirectRunner, and Cloud Composer orchestration (neither started).
